...
exports.fetchDatas = async (req, res) => {
    ...
    try {
        const { count, rows } = await CarouselModel.findAndCountAll({
            where: searchFilter,
            limit,
            offset,
            order: [['id', 'DESC']]
...
exports.fetchAllDatas = async (req, res) => {
    try {
        const datas = await CarouselModel.findAll({
            attributes: [
...
exports.fetchSigleData = async (req, res) => {
    try {
        const datas = await CarouselModel.findByPk(req.params.id);
...
exports.postData = async (req, res) => {
    ...
    try {
        if (!id || id === "") {
            ...
            // 🔸 Mise à jour
            const [updated] = await CarouselModel.update({ titre, description }, { where: { id } });
            if (updated) {
...
exports.deleteData = async (req, res) => {
    try {
        const deleted = await CarouselModel.destroy({ where: { id: req.params.id } });
        if (deleted) {
