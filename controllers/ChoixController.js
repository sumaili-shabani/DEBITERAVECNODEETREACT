const ChoixModel = require('../models/ChoixModel');
...
exports.fetchDatas = async (req, res) => {
    ...
    try {
        const { count, rows } = await ChoixModel.findAndCountAll({
            where: searchFilter,
            limit,
            offset,
            order: [['id', 'DESC']]
...
exports.fetchAllDatas = async (req, res) => {
    try {
        const datas = await ChoixModel.findAll({
            attributes: [
...
exports.fetchSigleData = async (req, res) => {
    try {
        const datas = await ChoixModel.findByPk(req.params.id);
...
exports.postData = async (req, res) => {
    ...
    try {
        if (!id || id === "") {
            // 🔸 Insertion
            await ChoixModel.create({ titre, description, icone});
            res.status(200).json({ message: "Insertion avec succès !!!" });
        } else {
            // 🔸 Mise à jour
            const [updated] = await ChoixModel.update({ titre, description, icone }, { where: { id } });
            if (updated) {
...
exports.deleteData = async (req, res) => {
    try {
        const deleted = await ChoixModel.destroy({ where: { id: req.params.id } });
        if (deleted) {
