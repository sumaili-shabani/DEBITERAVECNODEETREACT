...
exports.fetchAllDatas = async (req, res) => {
    try {
        const datas = await FaqModel.findAll({
            attributes: [
...
exports.fetchSigleData = async (req, res) => {
    try {
        const datas = await FaqModel.findByPk(req.params.id);
...
exports.postData = async (req, res) => {
    ...
    try {
        if (!id || id === "") {
            // 🔸 Insertion
            await FaqModel.create({ question, reponse });
            res.status(200).json({ message: "Insertion avec succès !!!" });
        } else {
            // 🔸 Mise à jour
            const [updated] = await FaqModel.update({ question, reponse }, { where: { id } });
            if (updated) {
...
exports.deleteData = async (req, res) => {
    try {
        const deleted = await FaqModel.destroy({ where: { id: req.params.id } });
        if (deleted) {
