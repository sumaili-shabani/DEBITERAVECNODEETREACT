const FonctionaliteModel = require('../models/Fonctionalite');
...
exports.fetchDatas = async (req, res) => {
    ...
    try {
        const { count, rows } = await FonctionaliteModel.findAndCountAll({
            where: searchFilter,
            limit,
            offset,
            order: [['id', 'DESC']]
...
exports.fetchAllDatas = async (req, res) => {
    try {
        const datas = await FonctionaliteModel.findAll({
            attributes: [
...
exports.fetchSigleData = async (req, res) => {
    try {
        const datas = await FonctionaliteModel.findByPk(req.params.id);
...
exports.postData = async (req, res) => {
    const { id, titre, description, icone } = req.body;
    try {
        if (!id || id === "") {
            // 🔸 Insertion
            await FonctionaliteModel.create({ titre, description, icone});
            res.status(200).json({ message: "Insertion avec succès !!!" });
        } else {
            // 🔸 Mise à jour
            const [updated] = await FonctionaliteModel.update({ titre, description, icone }, { where: { id } });
            if (updated) {
...
exports.deleteData = async (req, res) => {
    try {
        const deleted = await FonctionaliteModel.destroy({ where: { id: req.params.id } });
        if (deleted) {
