...
exports.fetchDatas = async (req, res) => {
    ...
    const searchFilter = search
        ? {
            icone: { [Op.like]: `%${search}%` }
        }
        : {};

    try {
        const { count, rows } = await GaleryMadel.findAndCountAll({
            where: searchFilter,
            limit,
            offset,
            order: [['id', 'DESC']]
...
exports.fetchAllDatas = async (req, res) => {
    try {
        const datas = await GaleryMadel.findAll({
            attributes: [
...
exports.fetchSigleData = async (req, res) => {
    try {
        const datas = await GaleryMadel.findByPk(req.params.id);
...
exports.deleteData = async (req, res) => {
    try {
        const deleted = await GaleryMadel.destroy({ where: { id: req.params.id } });
        if (deleted) {
