const TugModel = require('../models/TugModel');
...
exports.fetchDatas = async (req, res) => {
    ...
    try {
        const { count, rows } = await TugModel.findAndCountAll({
            where: searchFilter,
            limit,
            offset,
            order: [['id', 'DESC']]
...
exports.fetchSigleData = async (req, res) => {
    try {
        const datas = await TugModel.findByPk(req.params.id);
...
exports.postData = async (req, res) => {
    ...
    try {
        if (!id || id === "") {
            // 🔸 Insertion
            await TugModel.create({ titre});
            res.status(200).json({ message: "Insertion avec succès !!!" });
        } else {
            // 🔸 Mise à jour
            const [updated] = await TugModel.update({ titre}, { where: { id } });
            if (updated) {
...
exports.deleteData = async (req, res) => {
    try {
        const deleted = await TugModel.destroy({ where: { id: req.params.id } });
        if (deleted) {
