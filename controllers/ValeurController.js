const Valeur = require('../models/Valeur');
...
exports.fetchDatas = async (req, res) => {
    ...
    try {
        const { count, rows } = await Valeur.findAndCountAll({
            where: searchFilter,
            limit,
            offset,
            order: [['id', 'DESC']]
...
exports.fetchAllDatas = async (req, res) => {
    try {
        const datas = await Valeur.findAll({
            attributes: [
...
exports.fetchSigleData = async (req, res) => {
    try {
        const datas = await Valeur.findByPk(req.params.id);
...
exports.postData = async (req, res) => {
    ...
    try {
        if (!id || id === "") {
            // 🔸 Insertion
            await Valeur.create({ titre, description, icone});
            res.status(200).json({ message: "Insertion avec succès !!!" });
        } else {
            // 🔸 Mise à jour
            const [updated] = await Valeur.update({ titre, description, icone }, { where: { id } });
            if (updated) {
...
exports.deleteData = async (req, res) => {
    try {
        const deleted = await Valeur.destroy({ where: { id: req.params.id } });
        if (deleted) {
