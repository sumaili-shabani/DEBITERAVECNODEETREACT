...
export default function CategoryBlog() {
    ...
    const [listData, setDataList] = useState<UiCatBlog[]>([]);
    const [formData, setFormData] = useState<Partial<UiCatBlog>>({});
    ...
    const loadlistData = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<UiCatBlog>('/fetch_category_blog', {
                q: search,
                page: currentPage,
                limit,
            });
    ...
    */
    const handleEdit = async (id: number) => {
        const role = await fetchItem<UiCatBlog>('/fetch_single_category_blog', id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (confirmed) {
            try {
                await removeItem('/delete_category_blog', id);
                loadlistData();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        await saveItem('/insert_category_blog', formData);
        loadlistData();
        handleCloseModal();
    };
    ...
    return (
        <div className="container mt-4">
            <h4 className="mb-3">Liste des catégorie de blogs</h4>
            {/* loading component */}
            <LoaderAndError
                loading={loading}
                error={error}
            ...
            >
                <form onSubmit={handleSubmit}>
                    <TextField
                        ...
                        placeholder="Nom Catégorie blog"
                        label="Nom Catégorie blog"
                        icon="fas fa-heading"
                        required
                    />
