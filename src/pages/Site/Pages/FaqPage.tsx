...
export default function FaqPage() {
    ...
    const loadlistData = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<UiBasic>('/fetch_faq', {
                q: search,
                page: currentPage,
                limit,
            });
    ...
    */
    const handleEdit = async (id: number) => {
        const role = await fetchItem<UiBasic>('/fetch_single_faq', id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (confirmed) {
            try {
                await removeItem('/delete_faq', id);
                loadlistData();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        await saveItem('/insert_faq', formData);
        loadlistData();
        handleCloseModal();
    };
    ...
    return (
        <div className="container mt-4">
            <h4 className="mb-3">Liste des faq</h4>
            {/* loading component */}
            <LoaderAndError
                loading={loading}
                error={error}
                        ...
                        {listData.length === 0 ? (
                            ...
                            listData.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td>{truncateText(item.question!, 20)}</td>
                                    <td>{truncateText(item.reponse!, 20)}</td>
