...
export default function FonctionalitePage() {
    // declaration de variables
    const [listData, setDataList] = useState<UiValeur[]>([]);
    const [formData, setFormData] = useState<Partial<UiValeur>>({});
    ...
    const loadlistData = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<UiValeur>('/fetch_fonctionalite', {
                q: search,
                page: currentPage,
                limit,
            });
    ...
    */
    const handleEdit = async (id: number) => {
        const role = await fetchItem<UiValeur>('/fetch_single_fonctionalite', id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (confirmed) {
            try {
                await removeItem('/delete_fonctionalite', id);
                loadlistData();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        await saveItem('/insert_fonctionalite', formData);
        loadlistData();
        handleCloseModal();
    };
                        ...
                        <div className="row">
                            ...
                            <div className="col-md-6">
                                <TextField
                                    ...
                                    onChange={handleInputChange}
                                    placeholder="fas fa-text-width"
                                    label="Icone avec font"
                                    icon="fas fa-text-width"
                                    required
                                ...
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        <button type="button" className='btn btn-sm btn-secondary'>
                                            <i className={`${item.icone!} fa-lg`}></i>
                                        </button>
                                    </td>
