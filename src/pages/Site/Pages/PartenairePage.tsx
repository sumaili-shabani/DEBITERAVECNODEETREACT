...
export default function PartenairePage() {
    ...
    const [listData, setDataList] = useState<UiPartenaire[]>([]);
    const [formData, setFormData] = useState<Partial<UiPartenaire>>({});
    ...
    const loadlistData = async () => {
        ...
        try {
            const res = await fetchItems<UiPartenaire>('/fetch_partenaire', {
                q: search,
                page: currentPage,
                limit,
            });
    ...
    */
    const handleEdit = async (id: number) => {
        const role = await fetchItem<UiPartenaire>('/fetch_single_partenaire', id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (confirmed) {
            try {
                await removeItem('/delete_partenaire', id);
                loadlistData();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        await saveItem('/insert_partenaire', formData);
        loadlistData();
        handleCloseModal();
    };
    ...
    const handlEditImage = async (id: number) => {
        const data = await fetchItem<UiPartenaire>('/fetch_single_partenaire', id);
    ...
    const handleSubmitImage = async (e: React.FormEvent) => {
        ...
        await saveItemImageForm('/edit_partenaire_logo', formDataToSend);
        loadlistData();
    ...
    */
    ...
    return (
        <div className="mt-4">
            <h4 className="mb-3">Liste des sliders carousels</h4>
            {/* loading component */}
            <LoaderAndError
                loading={loading}
                error={error}
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-12">
                            <TextField
                                ...
                                label="Url du site de partenaire"
                                icon="fas fa-link"
                        ...
                        {listData.length === 0 ? (
                            ...
                            listData.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        <a href={item.url!} target='_blank'> <i className='fas fa-link fa-sm'></i> 
                                            {item.url != null ? truncateText(item.url!, 20):''}
