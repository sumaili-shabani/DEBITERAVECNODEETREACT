...
export default function ServicePage() {
    ...
    const loadlistData = async () => {
        setLoading(true);
        try {
            const res = await fetchItems<UiService>('/fetch_service', {
                q: search,
                page: currentPage,
                limit,
            });
    ...
    */
    const handleEdit = async (id: number) => {
        const role = await fetchItem<UiService>('/fetch_single_service', id);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (confirmed) {
            try {
                await removeItem('/delete_service', id);
                loadlistData();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        await saveItem('/insert_service', formData);
        loadlistData();
        handleCloseModal();
    };
    ...
    return (
        <div className="container mt-4">
            <h4 className="mb-3">Liste des services</h4>
            {/* loading component */}
            <LoaderAndError
                loading={loading}
                error={error}
